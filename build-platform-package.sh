...
cd "$package_source"
docker run -d --rm -v "$package_source:/source:ro" -v "$buildtmp:/target" --name "$container" "awoland/dsmpkg-env:$dsm_version-$dsm_platform" /bin/bash -c "trap : TERM INT; sleep infinity & wait"
make bootstrap
